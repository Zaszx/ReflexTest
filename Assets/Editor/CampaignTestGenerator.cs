...
public static class CampaignTestGenerator
{
    ...
    private const string GeneratorVersion = "campaign-generator-v2-variable-outlines";
    ...
    {
        level.stableId=$"neon-reflex-level-{number:000}";level.levelNumber=number;float progression=(number-1)/99f;level.gridSize=number<6?3:number<=10?4:number<18?3:number<52?4:5;level.outlineCount=number<=5?2:number<15?3:number<35?(number%4==3?4:3):(number%7==0?5:number%3==0?4:3);level.requiredCorrectClicks=2*(15+Mathf.RoundToInt((number-1)*65f/99f));level.timeLimit=Mathf.Lerp(14.4f,7.5f,progression);level.completionCoinReward=12+number*3;level.enemies=new EnemyLevelSettings { enabled=number==25||(number>=30&&number%5==0&&number!=35) };level.reverseEnabled=false;level.rotateSpeed=0;level.scaleEnabled=false;level.minimumGridScale=.9f;level.maximumGridScale=1f;level.scaleCycleDuration=5f;level.movementEnabled=false;level.movementSpeedNormalized=0;level.movementTravelPaddingNormalized=.03f;
