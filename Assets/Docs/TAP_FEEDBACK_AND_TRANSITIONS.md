...
Success particles remain disabled by `successParticlesEnabled` in `NeonMotionSettings.asset`; the implementation remains available for re-enabling. Smiley spawning, presentation, shader, settings and obsolete imported copies were removed in the recovery/enemy feature batch; the original root `FeedbackSmileys/` artwork is retained.
...
- Historical pre-removal EditMode result: **78 passed**. Historical smiley runtime captures: `Artifacts/Motion/SmileyFeedback/540x1200/` (normal success, mistake, and Reverse success).
