...
public static class CampaignTestGenerator
{
    ...
    {
        level.stableId=$"neon-reflex-level-{number:000}";level.levelNumber=number;int plateau=(number-1)/10;level.gridSize=number<6?3:number<=10?4:number<18?3:number<52?4:5;level.requiredCorrectClicks=2*(15+Mathf.RoundToInt((number-1)*65f/99f));level.timeLimit=.5f*Mathf.Max(13,32-plateau*1.45f+(number%5==0?4:0));level.completionCoinReward=12+number*3;level.reverseEnabled=false;level.rotateSpeed=0;level.scaleEnabled=false;level.minimumGridScale=.9f;level.maximumGridScale=1f;level.scaleCycleDuration=5f;level.movementEnabled=false;level.movementSpeedNormalized=0;level.movementTravelPaddingNormalized=.03f;
        if(number==2)level.reverseEnabled=true; else if(number==3)level.rotateSpeed=30f; else if(number==4){level.scaleEnabled=true;level.minimumGridScale=.92f;level.scaleCycleDuration=2.4f;}else if(number==5){level.movementEnabled=true;level.movementSpeedNormalized=.09f;}else if(number>=6){int pattern=(number-6)%4;float rotationMagnitude=30f+plateau*5f;if(number<=20){level.reverseEnabled=pattern==0||pattern==1;level.rotateSpeed=(pattern==0||pattern==2)?(number%2==0?1:-1)*rotationMagnitude:0;level.scaleEnabled=pattern==1||pattern==3;level.movementEnabled=pattern==2||pattern==3;}else if(number<=40){level.reverseEnabled=pattern!=0&&pattern!=3;level.rotateSpeed=pattern==0?0:(number%2==0?1:-1)*rotationMagnitude;level.scaleEnabled=pattern!=1;level.movementEnabled=pattern!=2;}else{level.reverseEnabled=true;level.rotateSpeed=(number%2==0?1:-1)*rotationMagnitude;level.scaleEnabled=true;level.movementEnabled=true;}if(level.scaleEnabled){level.minimumGridScale=Mathf.Max(.82f,.94f-plateau*.01f);level.maximumGridScale=1f;level.scaleCycleDuration=Mathf.Max(1.8f,2.6f-plateau*.08f);}if(level.movementEnabled)level.movementSpeedNormalized=.075f+plateau*.01f;}
    }
}
#endif
